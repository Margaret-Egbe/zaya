...
const FreeDelivery: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDeals = async () => {
      ...
      const freeDelivery = data.filter((product) => product.isFreeDelivery);
       setDelivery(freeDelivery as Product[]);
       setLoading(false);
    };
  ...
  return (
    <div className="flex overflow-x-auto gap-4 pb-4 scrollbar-thin scrollbar-thumb-white">
      {delivery.map((product) => (
        ...
        <div
          className="min-w-[170px] md:min-w-[180px] bg-white rounded-lg p-3 text-black shadow hover:shadow-lg cursor-pointer"
        >
          <img
